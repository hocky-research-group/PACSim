...
class TrajectoryWrapper:
    ...
    def subset_particles(self, indices: np.ndarray):
        ...
        if hasattr(frame, 'constraints') and hasattr(frame.constraints,
                                                     'group') and frame.constraints.group is not None:
            warnings.warn("Subsetting trajectories with constraints may lead to the creation of cluster fragments.",
                          UserWarning)
    ...
    def superset_particles(self, other: 'TrajectoryWrapper'):
        ...
        # Filter and remap constraints
        if (hasattr(other_frame, 'constraints') and hasattr(other_frame.constraints,
                                                            'group') and other_frame.constraints.group is not None
                and (hasattr(base_frame, 'constraints') and hasattr(base_frame.constraints,
                                                                    'group') and base_frame.constraints.group is not None)):
    ...
    def seed_particles(self, seed: 'TrajectoryWrapper', epsilon: float = 0, seed_fractional_coords: np.ndarray = None):
        ...
        if np.any(np.abs(np.mean(seed_positions, axis=0)) > (0.5 * current_box)):
            warnings.warn("Seed positions are outside half the current unit cell size. Particle overlap may occur.",
                          UserWarning)
