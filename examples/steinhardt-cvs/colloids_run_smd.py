...
def calc_stein_single(i: int, pos, num_nbs: int, order: int, device: torch.device):
    ...
    inds_nbs = torch.argsort(dists)[1:order+1]
    ...
    for n in range(order+1):
        ...
        q_n[n] = torch.sqrt(4*np.pi/(2*n+1)*q_n[n])
    return q_n[order]
...
class CV0Module(torch.nn.Module):
    def __init__(self, pindex, num_nbs, order):
        super().__init__()
    ...
    def forward(self, positions):
        ...
        q_n = calc_stein_single(self.pindex, positions, self.num_nbs, self.order,
                                self.device)
        
        return q_n

class CVReporter(object):
    def __init__(self, file, reportInterval, cv0):
    ...
    def report(self, simulation, state):
        ...
        self._out.write('%d %.6f\n' % (simulation.currentStep, cv0_val))
        self._out.flush()
...
def set_up_simulation(parameters: RunParameters, types: npt.NDArray[str],
    ...
    # Setting up Steinhardt parameter CV, hard-coding things for now...
    
    
    pindex = 640
    num_nbs = 6
    q_order = 6
    cv0module = torch.jit.script(CV0Module(pindex, num_nbs, q_order))
    ...
    pullingForce0.addGlobalParameter('r0', 0.75)
