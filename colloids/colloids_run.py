...
def set_up_reporters(parameters: RunParameters, simulation: app.Simulation, append_file: bool,
    ...
    if parameters.update_reporters is not None:
        for num, reporter in enumerate(parameters.update_reporters):
            update_reporter = getattr(update_reporters, reporter)
            try:
                reporter_index = num +1
                simulation.reporters.append(update_reporter(simulation=simulation, append_file=append_file,
                                                            **parameters.update_reporter_parameters[reporter_index]))
            except TypeError:
                raise TypeError(
                    f"UpdateReporter does not accept the given arguments {parameters.update_reporter_parameters[reporter_index]}. "
